...
export function HowWeWorkTimeline() {
  return (
    <div className="rounded-2xl bg-[color:var(--color-bg-primary)] p-6 md:rounded-3xl md:p-10">
      {/* Заголовок секции */}
      <div className="mb-8 text-center md:mb-10">
        <p className="mb-2 text-xs font-medium uppercase tracking-[0.22em] text-[color:var(--color-text-accent)]">
          Как мы работаем
        </p>
        <h2 className="text-2xl font-semibold leading-tight text-[color:var(--color-brand-forest-light)] md:text-3xl">
          Привезём твою вещь без риска
        </h2>
        <p className="mx-auto mt-3 max-w-sm text-sm text-[color:var(--color-text-secondary)]">
      ...
      <ol className="relative flex flex-col gap-0 md:flex-row md:items-start md:justify-between">
        {STEPS.map((step, index) => {
          ...
          return (
            <li
              key={step.label}
              className="relative flex flex-row items-start gap-4 pb-8 md:flex-1 md:flex-col md:items-center md:pb-0 md:text-center"
            >
              ...
              {!isLast && (
                <>
                  ...
                  <span
                    aria-hidden="true"
                    className="absolute left-[19px] top-10 h-full w-px bg-[color:var(--color-border-primary)] md:hidden"
                  />
              ...
              {/* Иконка */}
              <div className="relative z-10 flex h-10 w-10 shrink-0 items-center justify-center rounded-full border border-[color:var(--color-border-primary)] bg-[color:var(--color-bg-tertiary)] text-[color:var(--color-brand-forest)] md:h-10 md:w-10">
                <Icon className="h-4 w-4" />
              </div>

              {/* Текст */}
              <div className="pt-1 md:pt-3">
                <p className="text-[10px] font-semibold uppercase tracking-[0.14em] text-[color:var(--color-text-accent)] md:mb-0.5">
                  {String(index + 1).padStart(2, "0")}
                </p>
                <p className="mt-0.5 text-sm leading-snug text-[color:var(--color-text-primary)] md:mt-1 md:max-w-[120px]">
                  {step.text}
                </p>
              </div>
            </li>
      ...
      {/* CTA */}
      <div className="mt-10 flex flex-col items-center gap-3 border-t border-[color:var(--color-border-secondary)] pt-8 sm:flex-row sm:justify-center">
        <Link
          href="/contacts"
